...
function exitOnGoalCompletion(): GoalCompletionListener {
    return async (inv: GoalCompletionListenerInvocation) => {
        ...
        } else if (completedGoal.state === SdmGoalState.canceled || completedGoal.state === SdmGoalState.stopped) {
            logger.info("Exciting because '%s' was canceled or stopped", completedGoal.uniqueName);
...
function allSuccessful(goals: SdmGoalEvent[]): boolean {
    goals.forEach(g => logger.debug("Ggoal '%s' is %s", g.name, g.state));
    return !goals.some(g =>
