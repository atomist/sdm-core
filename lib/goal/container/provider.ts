...
export async function prepareSecrets(container: GoalContainer,
    ...
    if (container?.secrets?.env) {
        for (const secret of container.secrets.env) {
            if (!!secret.value.provider) {
                const value = await prepareProviderSecret(secret.value, gi, secrets, secret.name);
                if (!!value) {
    ...
    if (container?.secrets?.fileMounts) {
        for (const secret of container.secrets.fileMounts) {
            if (!!secret.value.provider) {
                const value = await prepareProviderSecret(secret.value, gi, secrets);
                if (!!value) {
...
async function prepareProviderSecret(secret: GoalContainerSecret["value"],
                                     gi: GoalInvocation,
                                     secrets: Secrets,
