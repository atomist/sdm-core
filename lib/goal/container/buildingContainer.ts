...
export class BuildingContainer extends FulfillableGoal {
    ...
    public async plan(pli: PushListenerInvocation, goals: Goals): Promise<PlannedGoals> {
        ...
        for (const container of reg.containers.filter(isBuildingContainer)) {
            ...
            // TODO cd are there more generic ways to checking if an image exists without docker pull?
            ...
            for (const i of images) {
                const registration: DockerContainerRegistration = {
                    ...
                    volumes: [{
                        name: "creds",
                        hostPath: {
                            // TODO cd switch this to using the DockerRegistryProvider from the graph
