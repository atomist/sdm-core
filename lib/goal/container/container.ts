...
export interface GoalContainerSpec {
    /**
     * Containers to run for this goal.
     */
...
export interface ContainerRegistration extends Partial<ImplementationRegistration>, GoalContainerSpec {
    ...
    /**
     * File path globs to retrieve from cache before starting goal
     * execution.  The values must correspond to output values from
     * previously executed container goals in the same goal set.
     */
    input?: CacheEntry[];
    /**
...
export class Container extends FulfillableGoalWithRegistrations<ContainerRegistration> {
    ...
    constructor(public readonly details: ContainerGoalDetails = {}, ...dependsOn: Goal[]) {
        super(getGoalDefinitionFrom(details, DefaultGoalNameGenerator.generateName("container")), ...dependsOn);
    }
    ...
    public withProjectListener(listener: GoalProjectListenerRegistration): this {
        throw new Error("GoalProjectListenerRegistration are not supported on Container goals");
    }
}
