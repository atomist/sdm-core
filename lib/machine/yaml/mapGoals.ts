...
export async function mapGoals(sdm: SoftwareDeliveryMachine,
    ...
    if (Array.isArray(goals)) {
        ...
        if (!!goals.containers) {
            ...
            goal = await mapGoals(sdm, await mapReferencedGoal(sdm, goals), additionalGoals, goalMakers, additionalTests, extensionTests);
        } else {
            for (const goalName in goals) {
                if (goals.hasOwnProperty(goalName) && goalName.includes("/")) {
                    const gd = goals[goalName];
                    const gn = gd?.name;
                    delete gd.name;
                    const referencedGoal = await mapReferencedGoal(sdm, goalName, gn);
                    if (!!referencedGoal) {
                        goal = await mapGoals(sdm, _.merge({}, referencedGoal, gd), additionalGoals, goalMakers, additionalTests, extensionTests);
                    }
                }
            }
...
async function mapReferencedGoal(sdm: SoftwareDeliveryMachine, goalRef: string, goalName?: string): Promise<any> {
    let owner;
    let repo;
    let ref = "master";
    let slug = goalRef;
    if (goalRef.includes("@")) {
        ref = goalRef.split("@")[1];
        slug = goalRef.split("@")[0];
    }
    owner = slug.split("/")[0];
    repo = slug.split("/")[1];
    const goalNames = !!goalName ? [goalName] :
        repo.endsWith("-goal") ? [repo, repo.replace(/-goal/, "")] : [repo];
    ...
    try {
        ...
        const documents = yaml.safeLoadAll(content);
        for (const document of documents) {
            for (const goalName in document) {
                if (document.hasOwnProperty(goalName) && goalNames.includes(goalName)) {
                    return document[goalName];
                }
            }
        }
    } catch (e) {
        throw new Error(`Referenced goal '${goalRef}' can not be found`);
    }
    return undefined;
}
