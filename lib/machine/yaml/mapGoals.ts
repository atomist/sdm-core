...
export async function mapGoals(sdm: SoftwareDeliveryMachine,
    ...
    if (Array.isArray(goalStructure)) {
        ...
        let goals = typeof goalStructure !== "string" ? camelcaseKeys(goalStructure, { deep: true }) : goalStructure as any;
        let goal;
        if (!!goals.containers) {
            const containers = [];
            ...
            for (const goalName in goalStructure) {
                if (goalStructure.hasOwnProperty(goalName) && goalName.includes("/")) {
                    const referencedGoal = await mapReferencedGoal(sdm, goalName);
                    if (!!referencedGoal) {
                        goal = await mapGoals(sdm, _.merge({}, referencedGoal, goalStructure[goalName] || {}), additionalGoals, goalMakers, additionalTests, extensionTests);
                    }
                }
            }
...
async function mapReferencedGoal(sdm: SoftwareDeliveryMachine, goalRef: string): Promise<any> {
    const regexp = /([a-zA-Z-_]*)\/([a-zA-Z-_]*)(?:\/([a-zA-Z-_]*))?@([a-zA-Z-_]*)/i;
