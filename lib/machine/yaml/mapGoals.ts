        ...
        if (!!goals.containers) {
            ...
            for (const goalMakerName in goalMakers) {
                if (!!goals[goalMakerName] || goalMakerName === goals) {
                    ...
                    if (!!goal) {
                        goal = addCaching(goal, goals[goalMakerName] || {});
                        break;
                    }
                }
            }
...
function addCaching(goal: GoalWithFulfillment, goals: any): Goal {
    if (!!goals.input && goals.input.length > 0) {
        goal.withProjectListener(cacheRestore({ entries: goals.input.map(c => ({ classifier: c })) }));
    }
    if (!!goals.output && goals.output.length > 0) {
        goal.withProjectListener(cachePut({ entries: goals.output }));
    }
    return goal;
}
