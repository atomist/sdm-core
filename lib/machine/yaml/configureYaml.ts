...
async function createGoalData<G extends DeliveryGoals>(patterns: string | string[],
    ...
    for (const file of files) {
        ...
        for (const config of configs) {
            ...
            await resolvePlaceholders(config, value =>
                resolvePlaceholder(value, undefined, { context: undefined, configuration: sdm.configuration }, false));
            ...
            for (const k in config) {
                if (config.hasOwnProperty(k)) {
                    ...
                    const goals = await mapGoals(sdm, v.goals, additionalGoals, goalMakers, options.tests || {}, testMakers);
...
async function resolveFiles(cwd: string, patterns: string | string[]): Promise<string[]> {
    ...
    process.env.ATOMIST_WATCH_PATHS = [...(process.env.ATOMIST_WATCH_PATHS || []), ...files].join(",");
    return files;
}
