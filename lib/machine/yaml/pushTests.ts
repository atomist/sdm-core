...
const And: CreatePushTest = async (test, additionalTests, extensionTests) => {
    if (test.and) {
        return and(await mapTest(test.and, additionalTests, extensionTests));
    }
    return undefined;
};
...
const Or: CreatePushTest = async (test, additionalTests, extensionTests) => {
    if (test.or) {
        return or(await mapTest(test.or, additionalTests, extensionTests));
    }
    return undefined;
};
...
export async function mapTest(test: any,
    ...
    for (const createPushTest of CreatePushTests) {
        const pushTest = await createPushTest(test, additionalTests, extensionTests);
        if (!!pushTest) {
            return pushTest;
        }
    }
