...
export async function mapTests(tests: any,
    ...
    return newTests.length > 0 ? allSatisfied(...newTests) : AnyPush;
}
...
export async function mapTest(test: any,
    ...
    if (test.hasFile) {
        ...
        return isMaterialChange({
            ...
            globs: toArray(test.isMaterialChange.patterns),
        });
    } else if (test.not) {
...
async function mapGoals(sdm: SoftwareDeliveryMachine,
    ...
    if (Array.isArray(goals)) {
        ...
        if (!!goals.containers) {
            const containers = [];
            for (const c of goals.containers) {
                containers.push({
                    ...c,
                    test: !!c.test ? await mapTests(c.test, additionalTests, extensionTests) : undefined,
                });
            }
            const c = container(
...
function containerCallback(): ContainerSpecCallback {
    return async (r, p, g, e, ctx) => {
        ...
        for (const container of r.containers) {
            const test = (container as any).test as PushTest;
