...
export async function configureYaml<G extends DeliveryGoals>(patterns: string | string[],
    ...
    return configure<G>(async sdm => {
        ...
        _.forEach(await createCommands(cwd), c => sdm.addCommand(c(sdm)));
        _.forEach(await createEvents(cwd), e => sdm.addEvent(e(sdm)));
...
async function createCommands(cwd: string, pattern: string[] = ["commands/**.js", "lib/commands/**.js"])
    : Promise<Record<string, (sdm: SoftwareDeliveryMachine) => CommandHandlerRegistration>> {
    const cmds: Record<string,  (sdm: SoftwareDeliveryMachine) => CommandHandlerRegistration> = {};
    const files = await resolveFiles(cwd, pattern);
    for (const file of files) {
        const testJs = require(`${cwd}/${file}`);
...
async function createEvents(cwd: string, pattern: string[] = ["events/**.js", "lib/events/**.js"])
    : Promise<Record<string, (sdm: SoftwareDeliveryMachine) => EventHandlerRegistration>> {
    const events: Record<string,  (sdm: SoftwareDeliveryMachine) => EventHandlerRegistration> = {};
    const files = await resolveFiles(cwd, pattern);
    for (const file of files) {
        const testJs = require(`${cwd}/${file}`);
