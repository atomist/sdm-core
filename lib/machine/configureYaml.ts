...
export async function configureYaml<G extends DeliveryGoals>(patterns: string | string[],
    ...
    return configure<G>(async sdm => {
        ...
        await awaitIterable(
            await createCommands(cwd, _.get(options, "extensions.commands")),
            async (c, k) => sdm.addCommand({ name: k, ...(await c(sdm)) }));
        await awaitIterable(
            await createEvents(cwd, _.get(options, "extensions.events")),
        ...
        const goalMakers = await createGoals(cwd, _.get(options, "extensions.goals"));
        const testMakers = await createTests(cwd, _.get(options, "extensions.tests"));
