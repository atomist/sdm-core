...
export async function configureYaml<G extends DeliveryGoals>(patterns: string | string[],
    ...
    const cfg: any = {};
    await awaitIterable(await requireConfiguration(cwd), async v => _.defaultsDeep(cfg, v));
    _.update(options, "options.preProcessors",
        old => !!old ? old : []);
    options.options.preProcessors = [async c => _.merge(c, cfg), ...toArray(options.options.preProcessors)];

    return configure<G>(async sdm => {

        await awaitIterable(
            await requireCommands(cwd, _.get(options, "extensions.commands")),
            async (c, k) => sdm.addCommand({ name: k, ...(await c(sdm)) }));
        await awaitIterable(
            await requireEvents(cwd, _.get(options, "extensions.events")),
        ...
        const goalMakers = await requireGoals(cwd, _.get(options, "extensions.goals"));
        const testMakers = await requireTests(cwd, _.get(options, "extensions.tests"));
...
async function mapGoals(sdm: SoftwareDeliveryMachine,
    ...
    if (Array.isArray(goals)) {
        ...
        if (!!goals.containers) {
            ...
            c.definition.preApprovalRequired = goals.approval;
