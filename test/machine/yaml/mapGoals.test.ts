...
describe("resolvePlaceholders", () => {
    ...
    it("should replace text in value", async () => {
        ...
        const result = await resolvePlaceholder(value, {} as any, { configuration: { color: "brown", attribute: "lazy" } } as any);
        assert.deepStrictEqual(result, "The quick brown fox jumps over the lazy dog");
    });

    it("should replace entire object", async () => {
        const value = "${colors}";
        const result = await resolvePlaceholder(value, {} as any, { configuration: { colors: { blue: "yes", orange: "nah"} } } as any);
