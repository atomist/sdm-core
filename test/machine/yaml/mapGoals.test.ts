...
describe("mapGoals", () => {
    ...
    it("should map goalMaker goal", async () => {
        ...
        const yaml = { sampleGoal: { input: "version", output: { target: { pattern: { directory: "target" } }} } };
    ...
    it("should map referenced goal", async () => {
        const yaml = "atomist/npm-goal/i-don't-exist@0.0.1";
        try {
            await mapGoals({ configuration: { http: { client: { factory: DefaultHttpClientFactory } } } } as any, yaml, {}, {}, {}, {});
            assert.fail();
        } catch (e) {
            assert.deepStrictEqual(e.message, "Unable to construct goal from '\"atomist/npm-goal/i-don't-exist@0.0.1\"'");
        }
    }).timeout(10000);
